...
`HairSalon.Solution/HairSalon/appsettings.json`
...
- (Terminal) In the working directory (HairSalon), type `$ dotnet run` to compile and exectute the server.  It will be available at [localhost5001](https://localhost:5001/) and [localhost5000](https://localhost:5000/)
...
- Also, the stylist needs to enter a photo url to submit the form, a good placeholder image to use is `https://picsum.photos/200`

## Known Bugs
* I made the form fills all require input so the user has to enter a value for each field to submit the form.  I noticed a bug in the Edit form for both Client and Stylist that if the user deletes the already set field value and submits the edit, it will save the value as an empty field. I did not want to make the edit inputs required, in the scenario they want to change 1 field, not all.  So if the user deletes the Name property and Saves, it is impossible to click on the entity to edit/delete/see details.
